...
std::filesystem::path GetFileName(auto lpFilename)
{
    ...
    static auto make_relative = [](const std::filesystem::path& path, const std::filesystem::path& base) -> auto {
        return std::filesystem::relative(std::filesystem::path(path).make_preferred(), base) / "";
    };
    ...
    if (std::filesystem::is_regular_file(filePath) || std::filesystem::is_directory(filePath) || std::filesystem::is_symlink(filePath))
    {
